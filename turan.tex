...
more than a year work on this problem and this work included computer
searches for solutions for small values of $n$.  Using the results of
these computer searches in the Online Encyclopedia of Integer Sequences
\cite{oeis}, we discovered that this problem is (asymptotically)
equivalent to several other known problems.
...
There is a simple and natural construction gives a lower bound of
$\Omega(n^{3/2})$ for these problems.  However, this bound is not
tight. A sequence of results has steadily raised this lower bound
\cite{tiskin,stein.szabo,hamaker.stein,}. The current record is held
by Gowers and Long \cite{gowers.long}, who describe a construction of
size $\Omega(n^{1.546})$.

The only known upper bounds for this problem come from the fact that
a solution to this problem gives a solution to the Rusza-Szemeredi
induced matching problem.  These upper bounds are barely subquadratic,
with the current record being held by Fox's improved version of the
triangle removal lemma \cite{fox}, which gives an upper bound of
$n^{2}/e^{\Omega(\log^* n)}$.  See, for example, Gowers and Long
\cite{gowers.long}

We point out a few bibliographic notes about these problems.  Bra\ss seems
to have been unaware that the question he posed was equivalent to tripod
packing and monotone matrices.

Tiskin, apparently unaware of the relation between tripod packing and
the Rusza-Szemeredi induced matching problem, also proved an upper bound
of $o(n^2)$. His proof uses tools (namely Szemeredi's Regularity Lemma)
that are the same as the original proofs of the Rusza-Szemeredia bounds.

Gowers and Long seem to be unaware that the problem on 2-comparable sets
of triples was studied under other names.

Gowers and Long arrived 2-comparable sets as a relaxation of a problem
(the size of the largest 2-increasing sequence of triples).  proposed
by Lo \cite{lo}.  In his discussion of this problem, Loh formulates
a restricted version of the induced matching problem, in which the
matching must satisfy a $\sigma$-free property and expresses hope
\cite[Remark~9]{loh} that this restricted version has an $O(n^{3/2})$
upper bound.  However, solutions for tripod packing correspond to
$\sigma$-free induced matchings, so $\sigma$-free induced matchings of
size $\omega(n^{3/2})$ are already known.
