...
\begin{lem}\lemlabel{survivor-structure}
  Let $S\subseteq Q$ be a set of points that have at least two distinct
  $x$-coordinates and at least two distinct $y$-coordinates.  Then,
  $\survivors(\{\disjointb\},S)$ consists of the triangular region
  $\{(x,y)\in Q: x<\ymin(S)\}$, the rectangular region $\{(x,y)\in Q:
  \xmax(S) <x <\ymax(S), ymax(S)\le y\le \xmax(S)\}$, and $O(1)$ additional points.  (See \figref{survivor-structure}.)
\end{lem}

\begin{figure}
  \begin{center}
      \includegraphics{figs/killers-11}
  \end{center}
  \caption{Illustration of \lemref{survivor-structure}. The structure of {$\survivors(\{\protect\disjointb\},S)$} consists mainly of a triangle determined by $\min(S)$ and a rectangle determined by $\ymax(S)$ and $\xmin(S)$.}
  \figlabel{survivor-structure}
\end{figure}

\begin{thm}
  $\ex(\edgea,\disjointb) \in O(n\log n)$.
\end{thm}

\begin{proof}[Proof sketch]
  Follows from \lemref{constant-degree} and the fact that the $\edgea$
  configuration prevents selecting the same point twice.
\end{proof}
