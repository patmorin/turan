...
the maximum number of triangles we can have while avoiding $X$
...
number of triangles obtained in the top-bottom view.
...
The following theorem say that we can find very large sets of triangles, all of which pairwise cross.
...
\begin{proof}
  Letting $S=\bigcup_{i=1}^n Q_i$ be the set of points played in
  a solution to the resulting dot-puzzle, we can observe that $S$
  does not contain any three points $a=(x_0,y_0)$, $b=(x_0,y_1)$, and
  $c=(x_1,y_1)$ with $y_0<y_1$ and $x_0<x_1$.  This implies that if we
  remove the rightmost point from every row of $S$, then at most one
  point remains in every column of $S$.  It immediately follows that $|S|\le 2n$.
\end{proof}

I believe \thmref{taco-nested-david} can be strengthened:

\begin{conj}\thmlabel{nested-david}
  $\ex'(n,\{\taco,\nested,\david\})\in O(n)$.
\end{conj}



\subsection{Some Easy Lower Bounds}

Finally, we clean up with some easy $\Omega(n^2)$ lower bound constructions.  In each case, a matching upper bound follows from one of the results in Bra\ss \cite{brass:turan}.

\begin{thm}\thmlabel{taco-david-bat-ears}
$\ex(n,\{\taco,\david\})\in\Theta(n^2)$.
\end{thm}

\begin{proof}
We set $Q_i$ to be all the points of $\{n/2,\ldots,n\}^2$ on the line $y=n/2-i-x$.
\end{proof}

\begin{thm}\thmlabel{taco-bat}
$\ex(n,\{\taco,\bat\})\in\Theta(n^2)$.
\end{thm}

\begin{proof}
We take $Q_i=\{(n/2+j,j):j\in\{1,\ldots,n/2\}\}$.
\end{proof}
...
\begin{proof}
We take $Q_i=\{(n/2+j,n/2-j):j\in\{1,\ldots,n/2\}\}$.
\end{proof}

\begin{thm}\thmlabel{bat-ears-nested-david}
...
\begin{thm}\thmlabel{crossing-david-ears}
  $\ex'(n,\{\crossing,\david,\ears\})\in \Omega(n^2)$.
\end{thm}

\begin{proof}[Proof Sketch]
  Downward diagonal on the square.
\end{proof}
...
\begin{thm}\thmlabel{nested-david-ears}\thmlabel{nested-david-ears}
  $\ex(n,\{\nested,\david,\ears\}) \in \Theta(n^2)$.
\end{thm}

\begin{proof}
   We set $Q_1=Q_2=\cdots=Q_n=\{(x,1): x\in\{1,\ldots,n\}$.
\end{proof}

\begin{thm}\thmlabel{taco-crossing}
  $\ex(n,\{\taco,\crossing\}) \in \Theta(n^2)$.
\end{thm}

\begin{proof}
   Diagonal downward stripes in the square.
\end{proof}



\begin{thm}\thmlabel{bat-david}
  $\ex(n,\{\bat,\david,\ears,\swords\}) \in \Theta(n^2)$.
\end{thm}
...
\begin{thm}\thmlabel{bat-ears}
  $\ex(n,\{\bat,\ears\}) \in \Theta(n^3)$.
\end{thm}

\begin{proof}
   We set $Q_1=Q_2=\cdots=Q_n=\{(x,y)\in Q: x>n/2, y<n/2\}$.
\end{proof}
