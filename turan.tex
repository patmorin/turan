...
\begin{tabular}{m{.55\textwidth}m{.4\textwidth}}
  ...
  $\on(n,\{\vertexc,\disjointb,\disjointc\}) \in \Omega(n^{3/2})$  
    & here (contradicts \cite{brass.rote.ea:triangles}) \\ \hline
  $\ex(n,\{\edgea,\vertexb,\vertexc\}) \in \Omega(n) \cap O(n\log n)$ & \thmref{blech} \\
  $\ex(n,X\cup\{\edgeb\})\in \Theta(\ex(n,X))$ & \lemref{xcup} \\
  \hline
  $\ex(n,\{\disjointa,\disjointb,\vertexa,\vertexb,\edgeb\}) \in \Theta(n)$ \newline
  $\on(n,\{\edgea,\disjointc,\vertexc\}) \in \Theta(n)$
    & \cite{brass.rote.ea:triangles} and \lemref{xcup} \\
  \hline
...
by various configurations.  The left column describes what happens
in subsequent rounds when the central (black) point is placed during
a round; none of the points marked with red crosses can be used in
subsequent rounds.  The right column shows what happens during the same
round; none of the points marked with red crosses can be selected during
the same round.  One caveat worth noting is that, unless $\edgea$ is a
forbidden configuration, the same point can be chosen in different rounds.
...
$\edgeb$ & \includegraphics[scale=.9]{figs/killers-1} \break% 
           $\{\}$  
         & \includegraphics[scale=.9]{figs/killersb-1} \break% 
           $\{(x',x)\} \cup \{(y,y')\}$ \\ 
$\edgea$ & \includegraphics[scale=.9]{figs/killers-2} \break% 
           $\{(x,y)\}$ 
         & \includegraphics[scale=.9]{figs/killersb-2} \break% 
           $\{(x,y')\} \cup \{(x',y)\}$ \\
$\vertexa$ & \includegraphics[scale=.9]{figs/killers-3} \break% 
             $\{(x',y) : x'<x \}\cup\{(x,y') : y'<y \}$ 
           & \includegraphics[scale=.9]{figs/killersb-3} \break%
             $\{(x',y'): x' < y\text{ or } y'> x\}$ \\
$\vertexb$ &  \includegraphics[scale=.9]{figs/killers-4} \break%
              $\{(x',y) : x'<x \}\cup\{(x,y') : y'<y \}$ 
         &  \includegraphics[scale=.9]{figs/killersb-4} \break%
            $\{(x',y'):\sign(x'-x)=\sign(y-y')\}$ \\
$\vertexc$ &  \includegraphics[scale=.9]{figs/killers-5} \break% 
              $\{(x',y) : x'>x \}\cup\{(x,y') : y'>y \}$ 
         &  \includegraphics[scale=.9]{figs/killersb-5} \break% 
            $\{(x',y'):\sign(x'-x)=\sign(y'-y)\}$ \\
$\disjointa$ &  \includegraphics[scale=.9]{figs/killers-6} \break%
                $\{(x',y'): x'< y\}$
         & \includegraphics[scale=.9]{figs/killersb-6} \break%
           $\{\}$ \\
$\disjointb$ & \includegraphics[scale=.9]{figs/killers-7} \break%
               $\{(x',y'): x'>x, y'< y\}$
         & \includegraphics[scale=.9]{figs/killersb-7} \break%
           $\{\}$ \\
$\disjointc$ &\includegraphics[scale=.9]{figs/killers-8} \break%
               $\{(x',y'): y < y' <x,\,\, x'>x\}$
         & \includegraphics[scale=.9]{figs/killersb-8} \break%
           $\{\}$ \\
\end{tabular}
\end{center}
...
From \tabref{forbidden}, some previous upper bounds naturally fall out. For
example, Bra\ss's results \cite{brass:turan} that $\ex(\vertexb)$ and
$\ex(\vertexc)$ are each $O(n^2)$ come from the fact that, during a single
round of the dot puzzle, the $y$ coordinates of the points must form
an non-decreasing (respectively, non-increasing) sequence when sorted
by $x$-coordinates.  This immediately implies that one can select at
...
Similarly, we can almost recover the result of Bra\ss, Rote and Swanepoel
on $\ex(\disjointa, \disjointb, \vertexa,\vertexb)$. From the forbidden
configurations $\vertexa$, $\vertexb$, and $\disjointa$, we obtain the
following rule
\begin{center}
  \includegraphics{figs/killers-3}
\end{center}
for which points are disallowed in subsequent rounds.  From $\vertexb$, we have the rule
\begin{center}
  \includegraphics{figs/killersb-4} \enspace .
\end{center}
Together, these two rules tell us that the points $\bigcup_{i=1}^n
Q_i$ must have $y$ coordinates that form a non-decreasing sequence
when ordered $x$-coordinate.  Therefore, $|Q_i|\le 2n-2$.  The bound
$\ex(\disjointa, \disjointb, \vertexa,\vertexb)\in O(n\log n)$ then
follows from \lemref{top-bottom}.
