...
\]
This is because the set $X$ in this case forbids any form of crossings
between the edges of triangles. Thus, the number maximum number of
triangles we can have while avoiding $X$ is the number of triangles in
a triangulation of a convex $n$-gon, i.e., $n-2$.
...
\begin{proof}
  ...
  $S'\subseteq S$ by removing any triangle that has a directed edge
  for which the triangle is to the left of the edge.  Observe that the
  set $S'$ does not contain any pair of triangles forming an $\edgeb$
  configuration.
  ...
  clockwise.  By linearity of expectation, $\E[|S'|]\ge|S|/8=\ex(n,X)/8$.
  We conclude therefore that there exists some subset $S''\subseteq S$
  of size least $\ex(n,X)/8$ that does not contain a pair of triangles
  that form an $\edgeb$ configuration.  The set $S''$ proves that
  $\ex(n,X\cup\{\edgeb\}) \ge \ex(n,X)/8$.
\end{proof}
...
horizontal line into a \emph{bottom half} of size $\lfloor n/2\rfloor$ and
...
\begin{thm}\thmlabel{blech}
  $\ex(n,\{\edgea,\vertexb,\vertexc\}) \in \Theta(n)$
\end{thm}

\begin{proof}
  We claim that in any set, $S$, of triangles that avoids
  \{\edgea,\vertexb,\vertexc,\edgeb\}, each vertex takes part in at most two
  triangles, which establishes the claim. To see this, consider some
  vertex $u$ and, without loss of generality, assume $u$ has minimum $y$
  coordinate among all vertices.  Among all triangles incident on $u$,
  consider the triangles $t_1$ (respectively, $t_2$) first struck by a
  downward vertical ray that originates at $u$ and then rotates clockwise
  (respectively, counterclockwise).  There are two cases to consider:
  \begin{enumerate}
    \item $t_1=t_2$.  In this case, any other triangle incident on $u$ either shares an edge with $t_1$, which is forbidden, or it forms a $\vertexb$ configuration with $t_1$, which is also forbidden.  Therefore, in this case, $t_1$ is the only triangle incident on $u$.

    \item $t_1\neq t_2$.  In this case, $t_1$
  \end{enumerate}
\end{proof}
