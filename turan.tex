...
\begin{table}
  ...
  For the new bounds listed as $n$, the lower bound is $\Omega(n)$ and the upper bound is $O(n\log n)$.  For the bounds in green, listed as $n^{1.546}:n^2$, the upper bound is actually $n^2/e^{\Omega(\log^* n)}$.}
  \tablabel{bigtable}
\end{table}
...
\Figref{forbidden-colour}.b shows the constraints placed on the locations
...
\begin{proof}
  ...
  if $Q_i$ contains $k_i$ vertices $v_1\prec\cdots\prec v_{k_i}$ vertices
