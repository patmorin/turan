...
\Figref{forbidden-color} summarizes the constraints on the puzzle induced by
various forbidden configurations.  The left side of the figure shows
restrictions on the locations of points placed during a single round.
It is interpreted as follows:  If the black central point, $p$, is placed
during round $i$, and we wish to avoid some particular configuration,
$c$, then we should not place any points in the part of the figure that
has the same colour as $c$.  For instance, if we wish to avoid the
configuration $c=\edgea$ configuration, then we should not place any
points in the same row or column as the central point; such a point
would correspond to a $\edgea$ configuration in which the shared edge joins
a bottom vertex to a left (same row) or right (same column) vertex.

The right column of \figref{forbidden-color} shows the constraints placed on the
locations of points placed in subsequent rounds.  Its interpretation is
similar to the the left side of the figure. For example, if we wish to
avoid a $\vertexb$ configuration and we place the central point, $p$,
during round $i$, then, in every round $j>i$, we should not place any
point directly to the left or directly below $p$.  Any such point would
correspond to a $\vertexb$ configuration in which the shared vertex is
the left vertex (to the left of $p$) or the right vertex (below $p$)
of both triangles.
...
From \figref{forbidden-color}, some previous upper bounds naturally fall out.
...
of \figref{forbidden-color}.  This immediately implies that one can select at
...
a non-decreasing point set.  From the forbidden configurations $\vertexa$,
...
\begin{obs}\obslabel{disjointb-region}
  For any non-empty set $S\subset Q$, $\survivors(\{\disjointb\},S)$
  is contained in the union of a \emph{triangular region} $T=\{(x,y)\in
  Q: x\le \xmin(S)+1\}$, a \emph{rectangular region} $R=\{(x,y)\in Q:
  x\ge \xmax(S),\, y\ge \ymax(S)\}$, a \emph{vertical line} $V=\{(x,y)\in Q:
  x=\xmin(S)\}$ and a \emph{horizontal line} $H=\{(x,y)\in Q: y=\ymin(S)\}$.
\end{obs}

\obsref{disjointb-region} gives us a form of \emph{potential function}
...
we define $T_i$, $R_i$, $V_i$, and $H_i$ as in \obsref{disjointb-region},
but with respect to the set $S=S_i$.

\begin{thm}\thmlabel{disjointb-edgea}
  $\ex(n,\{\disjointb,\edgea\}) \in O(n\log n)$.
\end{thm}

\begin{proof}
  We will directly bound $\sum_{i=1}^n |Q_i|$.  We begin by noting that,
  since we forbid $\edgea$, the set of points in each $Q_i$ contains
  ...
  \]
  We have just shown that $\ex'(n,,\{\disjointb,\edgea\})\in O(n)$,
  so the theorem follows from \lemref{top-bottom}.
\end{proof}

\begin{thm}\thmlabel{disjointb-vertexa}
  $\ex(n,\{\disjointb,\vertexa\}) \in \Theta(n^2)$.
\end{thm}

\begin{proof}
   The upper bound follows from the $O(n^2)$ upper bound on
   $\ex(n,\{\disjointb\})$ \cite{brass:turan}.
   For the lower-bound, we set $Q_1=\{(x,y)\in Q: x>n/2, y<n/2\}$ and
   set $Q_2=Q_3=\cdots Q_n=\emptyset$.
\end{proof}

\begin{thm}\thmlabel{disjointb-vertexb}
  $\ex(n,\{\disjointb,\vertexb\}) \in O(n\log n)$.
\end{thm}

\begin{proof}
   The proof is similar to the proof of \thmref{disjointb-edgea}.
   The forbidden configuration $\vertexb$ implies that each set $Q_i$
   must be non-decreasing (see the left side of \figref{forbidden-color}.
   ...
   $\survivors(\{\disjointb\},Q_i)$ in $H_i\setminus T_i\setminus R_i$.

   Summing everything, we find that
   $\ex'(n,\{\disjointb,\vertexb\})\le\sum_{j=1}^n |Q_i|\le 10n$, and
...
\begin{thm}
  $\ex(n,\{\disjointb,\disjointa,\disjointc\}) \in \Theta(n^2)$.
\end{thm}
