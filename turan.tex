...
and any $S\subset Q$, we define $\killed(X,S)$ as the subset of $Q$
that can no longer be played in the dot-puzzle game (for configurations
in $X$) if the points is $S$ have been played in previous rounds
(these are the points of $Q$ $\kappa$illed by $S$).  We also define
...
\begin{lem}\lemlabel{swords-region}
  ...
  subset of $Q$, let $S^{>}= \{(x,y)\in S : y > \xmin(S)\}$, and let
  $S^\le = S\setminus S^>$.
  Then $\survivors(\{\swords\},S)$ is contained in the union of
  a \emph{triangular region} $T=\{(x,y)\in Q: x\le \ymin(S)\}$,
  a \emph{rectangular region} $R=\{(x,y)\in Q: \xmax(S^\le) \le x \le \ymin(S^>),\,
  \ymax(S^\le) < y\le \xmin(S)\}$, a \emph{vertical line} $V_1$ passing
  through the rightmost point of $S^\le$ with maximum $y$-coordinate, vertical
  line passing through a point of $S^>$ with minimum $y$-coordinate, a
...
\begin{proof}
  The proof of \lemref{swords-region} is just a matter of checking that,
  for any $p\not\in T\cup R\cup H^{\max}\cup H^{\min}\cup V_1\cup V_2$,
  $p\in\killed(\{\swords\},q)$ for one of the (at most five) points $q$
  that determine $\xmin(S)$, $\xmax(S^\le)$, $\ymin(S)$, $\ymax(S^\le)$
  or $\ymin(S^>)$.  In performing this verification, it is helpful to keep
  in mind that
  \[  \ymin(S) \le \ymax(S^\le) \le \xmin(S) \le \ymin(S^>) \enspace . \] 
  Do note, however, that (unlike the situation illustrated in
  \figref{survivors}), it is possible that $\ymin(S^>) < \xmax(S^\le)$
  in which case the rectangle $R$ is empty.
\end{proof}

We will be using \lemref{swords-region} to study the evolution of the
dot-puzzle over $n$ rounds.  We will apply it with $S=\bigcup_{j=1}^i
Q_j$ to study the evolution $T$ and $R$ as rounds proceed.  So that we
can refer to these point sets as they change, we let $S_i=\bigcup_{j=1}^i
Q_i$ and we define $S_i^>$, $S_i^\le$, $T_i$, $R_i$, $V_i$, $H^{\max}_i$,
and $H^{\min}_i$ as in \lemref{swords-region}, but with respect to the
set $S=S_i$.

The evolution of $T_i$ over time is simple, it decreases as $\ymin(S)$
decreases.  However, the changes to $R_i$ are more complex. Sometimes
$R_i$ shrinks because $\xmin(S)$ decreases or because $\xmax(S^\le)$ or
$\ymax(S^\le)$ increases.  At other times, however, the rectangle $R_i$
makes a jump to a completely new location. The following lemma explains
these changes:

\begin{obs}\obslabel{shrink-jump}
  Let $R_1,\ldots,R_n$ be defined as above.  Then, for any $1\le i
  < j\le n$, $R_j \subseteq R_i$ or $\xmin(S_j) < \ymax(S_i^\le)$.
  In particular, in the latter case, $R_i$ and $R_j$ have no $x$- or
  $y$-coordinate in common.
\end{obs}

To see the utility of this observation, consider the index set
$I=\{i\in\{2,\ldots,n\}: R_i\not\subseteq R_{i-1}\}$ and define
$\Phi(R_i)=\ymin(S_i^>)-\ymax(S_i^\le) + \xmin(S_i) - \xmax(S_i^\le)$
(the sum of the width and height of $R_i$).  Then \obsref{shrink-jump}
tells us that $\sum_{i\in I}\Phi(R_i) < 2n$. 
...
\begin{proof}
  ...
  Similarly, if $Q_i$ contains $r_i\ge 1$ points in $R_i$ then one of
  two things happens: If $R_{i+1}\subset R_i$, then $\Phi(R_{i+1})\le
  \Phi(R_i)-r_i+1$ since all but one point of $Q_i\cap R_i$ kills an entire row
  or column of $R_i$. Otherwise, $R_{i+1}\not\subseteq R_i$.  Letting $I$
  denote the index set in which the second case occurs, we have:
  \[
      \sum_{i=1}^n (r_i-1) \le \sum_{i\in I}\Phi(R_i) < 2n \enspace .
  \]
  Thus, $\sum_{i=1}^n r_i < 3n$.
  
  Finally, we observe that $Q_i$ contains at most one point each from
  $H^{\min}_i$, $H^{\max}_i$, $V^1_i$, and $V^2_i$, so
  \[
      \sum_{i=1}^{n}|Q_i| \le \sum_{i=1}^n(t_i+r_i+4) \le 10 n \enspace . \qedhere
  \]
\end{proof}
...
\begin{proof}
   ...
   that $Q_i$ is non-decreasing means that $\Phi(R_{i+1}) \le \Phi(R_i)-1$
   or that $R_{i+1}\not\subseteq R_i$.  As in the previous proof, this implies that 
   $\sum_{i=1}^n r_i \le 3n$.
