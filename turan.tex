...
Boris Aronov\thanks{%
    ...
    CCF-15-40656, and by BSF grant 2014/170. Department of Computer
    Science and Engineering, Tandon School of Engineering, New York
    University, USA. },\,
Vida Dujmović\thanks{%
    Partially supported by NSERC and the Ontario Ministry of Research
    and Innovation. Department of Computer Science and Electrical Engineering,
...
Aurélien Ooms\thanks{%
    ...
    Agriculture (FRIA).  Département d'Informatique, Université libre 
    de Bruxelles (ULB), Belgium.  %\texttt{aureooms@ulb.ac.be}
},\,\\
and Luís Fernando Schultz Xavier da Silveira\footnotemark[2]
}
...
tight bounds are coloured blue, and gaps between lower and upper bounds
...
\begin{table}
  ...
  New results are in dark(er) green.}
...
easy lower bound constructions that are required to complete our table.
...
\[
    \sum_{i=1}^n|Q_i| \le 3n-4 \borisspace ,
\]
...
configurations of points.  We that three points $a=(x_0,y_0)$,
...
\begin{proof}
  ...
  $c$ forming a $\Gamma$-configuration. Refer
...
\begin{proof}
  ...
  and every point to the right of $p$ is killed by $p$ or cannot be included in
  $Q_i$ because of the rule for $\crossing$.  Therefore, $Q_i$ 
  eliminates at least $|Q_i|-1$ points of $S$. It follows that
...
\begin{proof}
   ...
   Define
   \[
        Q_i^* = \{ (x,y)\in Q_1 : x\ge\ymax(Q_1) \} \borisspace 
   \]
...
We begin with another lemma about forbidden configurations of points that
is similar to \lemref{forbidden}.  
We say that a point $(x_i,y_i)$ \emph{SE-dominates} a point $(x_j,y_j)$
if $x_i > x_j$ and $y_i < y_j$. 
We say that three points $a=(x_0,y_0)$,
$b=(x_0,y_1)$, and $c=(x_1,y_1)$ form a \emph{lazy-$L$-configuration}
if $y_1>y_0$ and $c$ SE-dominates $b$.
...
\begin{lem}\lemlabel{forbidden-ii}
   ...
   points $a$, $b$, and $c$ forming a lazy-$L$-configuration.  Then $|S|\le 2n$.
\end{lem}

\begin{proof}
  Refer to \figref{se-domination}.
  Consider the \emph{Pareto boundary}
  ...
  so that $a$, $b$, and $c$ would form the lazy-$L$-configuration.
\end{proof}
...
\begin{proof}
  ...
  $S$ contains no lazy-$L$-configuration so, by \lemref{forbidden-ii},
  $|S|\le 3n$.

  Suppose there were $a\in Q_i$, $b\in Q_j$, and $c\in Q_k$ forming a
  lazy-$L$-configuration.  Now, $a\in\killed(\{\swords\}, \{c\})$ and
...
\begin{proof}
   ...
   greedily 2-colour $G[A]$ by colouring $v_1,\ldots,v_{|A|}$ in order and,
  ...
  Finally, we are left with the complete acyclic subgraph $H'$ with vertex
  set $S_j'$ and whose topological sort order we denote by $\prec$.  Now,
  if $Q_i$ contains $k_i$ vertices $v_1\prec\cdots\prec v_{k_i}$ 
  of $H'$, then $v_1$ kills all of $v_2,\ldots,v_{k_i}$ so that these
  vertices can not appear in any $Q_{i'}$ with $i'>i$.  This implies that
  \[
      \sum_{i=1}^n (|S_j'\cap Q_i|-1) =
      \sum_{i=1}^n (k_i-1) \le |S_j'| \borisspace ,
  \]
...
The only known upper bound for this problem come from the fact that a
...
Known upper bounds for the induced matching problem are barely subquadratic,
...
Bra\ss \cite{brass:turan}.  The following are essentially ``proofs by figure'' in which a brief description of the solution is illustrated alongside the rules of each dot puzzle.
...
we take $Q_i$ to be all the points of $Q'=\{n/2,\ldots,n\}^2$ on the line $\{(x,y):y=3n/2-x-i+1\}$.
\end{proof}
