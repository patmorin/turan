...
\begin{proof}
  ...
  in $Q_i$ are non-increasing and the elements in $Q_{i,j}$ and $Q_{i,j+1}$
  share at most one $x$- or $y$-coordinate.  With this observation, the
...
\begin{proof}
  ...
  from that row.)  Therefore $|S|\le n$.  All that remains to account for
...
Our next two upper bounds depend on a simple lemma about forbidden
...
\begin{proof}
  ...
  to \figref{forbidden}.  Suppose, for the sake of contradition,
...
\begin{proof}
   ...
   (due to $\nested$) that $Q_j$ is non-decreasing since the leftmost
