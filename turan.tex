...
\author{Boris Aronov, Vida Dujmović, Ruy Fabila, Pat Morin, Aurélien Ooms,\\
...
Let $t_1$ and $t_2$ be a pair of distinct triangles whose (4 to 6) vertices
...
\begin{figure}
   \begin{center}
      ...
      \begin{tabular}{c@{\hspace{1cm}}c}
        ...
        \includegraphics[width=.48\ka]{figs/partition-1} &
        \includegraphics[width=.48\ka]{figs/partition-2} \\
        (a) & (b)
      \end{tabular}
   \end{center}
...
a non-decreasing point set.  The rules for points allowed in subsequent rounds
ensure
...
linear upper bounds bounds on $\ex'(n,\{\swords, c\})$ for each
...
For a point $q\in Q$, $\killed(\{\swords\}, \{q\})$ is a fairly large
region consisting (roughly) of three quadrants surrounding $q$ and
an additional triangular region:  
\begin{center}
  \includegraphics[height=3cm]{figs/swords-kappa}
\end{center}
However, $\killed(\{\swords\}, \{q\})$
has a fairly complicated shape, so much of the work in this section is
devoted to understanding, approximately, how $\killed(\{\swords\},S)$
evolves as points are added to the point set $S$.

In the following, for a point $q\in Q$, we use the notations $\x(q)$ and
$\y(q)$ to denote the $x$- and $y$-coordinates of $q$.  For $S\subseteq
Q$, we use $\xmin(S)$ to denote the minimum $x$-coordinate of any point
in the point set $S$, i.e., $\xmin(S)=\min\{\x(q) : q\in S\}$.  If $S$
is empty, then we define $\xmin(S)$ as $n+1$.  We define $\ymin(S)$,
$\xmax(S)$, and $\ymax(S)$ similarly, except that $\ymin(\emptyset) =
n$, $\ymax(\emptyset)=0$, and
 $\xmax(\emptyset)=1$.


\begin{lem}\lemlabel{swords-region}
  (Refer to \figref{survivors}.)  Let $S$ be any non-empty
  subset of $Q$, let $A = \{(x,y)\in S : y > \xmin(S)\}$, and let
  $B = S\setminus A$.
  Then $\survivors(\{\swords\},S)$ is contained in the union of
  a \emph{triangular region} 
  \[ T=\{(x,y)\in Q: x\le \ymin(S)\} \enspace , \]
  a \emph{rectangular region} 
  \[ R=\{(x,y)\in Q: \xmax(B) \le x \le \ymin(A),\, 
                 \ymax(B) < y\le \xmin(S)\} \enspace , \]
  a \emph{vertical line} $V^1$ passing
  through the rightmost point of $B$ with maximum $y$-coordinate, 
  a vertical
  line passing through a point of $A$ with minimum $y$-coordinate, a
  \emph{horizontal line} $H^{\min}=\{(x,y)\in Q: y=\ymin(S)\}$ and a
  horizontal line $H^{\max}=\{(x,y)\in Q: y=\ymax(B)\}$.
\end{lem}

\begin{figure}
  \begin{center}
    \includegraphics{figs/swords}
  \end{center}
  \caption{An illustration of \lemref{swords-region}.}
  \figlabel{survivors}
\end{figure}

\begin{proof}
  The proof of \lemref{swords-region}, which is illustrated already in
  \figref{survivors}, is just a matter of checking that,
  for any point 
  \[  p\in Q\setminus (T\cup R\cup H^{\max}\cup H^{\min}\cup V^1\cup V^2)  
      \enspace ,\]
  $p\in\killed(\{\swords\},q)$ for one of the (at most five) points $q$
  that determine $\xmin(S)$, $\xmax(B)$, $\ymin(S)$, $\ymax(B)$
  or $\ymin(A)$.  In performing this verification, it is helpful to keep
  in mind that
  \[  \ymin(S) \le \ymax(B) \le \xmin(S) \le \ymin(A) \enspace . \] 
  Note, however, that (unlike the situation illustrated in
  \figref{survivors}), it is possible that $\ymin(A) < \xmax(B)$
  in which case the rectangle $R$ is empty.
\end{proof}

We will be using \lemref{swords-region} to study the evolution of
dot-puzzle involving $\swords$ over $n$ rounds.  We will apply it
with $S=\bigcup_{j=1}^i Q_j$ to study the evolution $T$ and $R$ as
rounds proceed.  So that we can refer to these point sets as they
change, we let $S_i=\bigcup_{j=1}^i Q_i$ and we define $A_i$, $B_i$,
$T_i$, $R_i$, $V^1_i$, $V^2_i$, $H^{\max}_i$, and $H^{\min}_i$ as in
\lemref{swords-region}, but with respect to the set $S=S_i$.

The evolution of $T_i$ over time is easy to understand: it shrinks when
$\ymin(S_{i+1}) < \ymin(S_i)$.  However, the changes to $R_i$ are
more complex. Sometimes $R_i$ shrinks because $\xmin(S_{i+1})>\xmin(S_i)$,
$\xmax(B_{i+1})>\xmax(B_i)$ or $\ymax(B_{i+1})>\ymax(B_i)$.
Other times, however, $R_i$ makes a jump to a completely new location. The
following lemma explains these changes:

\begin{lem}\lemlabel{shrink-jump}
  For each $i\in\{1,\ldots,n\}$, let $Q_i$, $S_i$, $A_i$, $B_i$, and $R_i$
  be defined as above.  Then, for any $1\le i < j\le n$, $R_j \subseteq
  R_i$ or $\xmin(S_j) < \ymax(B_i)$.  In particular, in the latter case,
  $R_i$ and $R_j$ have no $x$- or $y$-coordinate in common.
\end{lem}

\begin{proof}
  Assume $\xmin(S_j) \ge \ymax(B_i)$, otherwise there is nothing to prove.
  Let $a\in B_i$ be a point with $\y(a)=\ymax(B_i)$.  Then $\y(a) =
  \ymax(B_i) \le \xmin(S_j)$ so $a\in B_j$.  Let $b\in B_i$ be a point
  with $\x(b)=\xmax(B_i)$.  Then $\y(b) \le \ymax(B_i) \le \xmin(S_j)$,
  so $b\in B_j$.

  Because $a,b\in B_j$, we have
  \[  \ymax(B_j) \ge \ymax(B_i) , \qquad \xmax(B_j) \ge \xmax(B_i) \enspace . \]
  Because $S_j\supseteq S_i$, it follows that $A_j\supseteq A_i$ and 
  \[
      \xmin(S_j) \le \xmin(S_i), \qquad \ymin(A_j) \le \ymin(A_i) \enspace .
  \]
  The left-hand sides of these of these four inequalities determine the
  four sides of $R_j$, while the right-hand sides determine the four
  sides of $R_i$, so $R_i\subseteq R_j$, as required.
\end{proof}

For a sequence $Q_1,\ldots,Q_n$ we say that $i\in\{1,\ldots,n-1\}$
is a \emph{critical event} if $\xmin(S_{i+1}) < \ymax(B_i)$.
\lemref{shrink-jump} says $R_i\subseteq R_{i+1}$ except when $i$ is
a critical event.  We use this to define a \emph{potential function}, $\Phi$,
on $R$ that decreases except during critical events.  The following lemma
defines this potential function and upper-bounds the size of these increases.

\begin{lem}\lemlabel{potential-r}
  For each $i\in\{1,\ldots,n\}$, let $Q_i$, $S_i$, $A_i$, $B_i$, and $R_i$
  be defined as above and define \[  \Phi(R_i)=\ymin(A_i)-\xmax(B_i) + 1 +
  \xmin(S_i) - \ymax(B_i) \] (the number of rows and columns of $R_i$).
  Let $I$ be the set of critical events.  Then $\sum_{i\in I}\Phi(R_i)
  < 2n$.
\end{lem}

\begin{proof}
  This follows immediately from \lemref{shrink-jump} since this lemma
  implies that, for any $i,j\in I$, $R_i$ and $R_j$ have no $x$-coordinate
  (row) or $y$-coordinate (column) in common. Therefore, since $Q$
  contains $n-1$ columns and $n$ rows, $\sum_{i\in I}\Phi(R_i) \le 2n-1$.
\end{proof}

We now have all the pieces we need to prove our linear upper bounds.
...
\begin{proof}
  ...
  First we count the contribution of points that are placed in
  $T_1,\ldots,T_{n-1}$. Let $t_i=|Q_{i+1}\cap T_i|$ and observe that
  $\ymin(S_{i+1}) \le \ymin(S_i)-t_i$.  This immediately implies that
  $\sum_{i=1}^{n-1} t_i \le n$.

  Now we count the contribution of points that are placed in
  $R_1,\ldots,R_{n-1}$.  Let $r_i=|Q_{i+1}\cap R_i|$.  Using the notation
  of \lemref{potential-r}, we know that $r_i \le Phi(R_i)$.  Furthermore,
  if $i$ is not a critical event, then $\Phi(R_{i+1})\le \Phi(R_i)-r_i+1$
  since all but one point of $Q_{i+1}\cap R_i$ kills an entire row or
  column of $R_i$.
  Therefore, by \lemref{potential-r}:
  \[
      \sum_{i=1}^{n-1} (r_i-1) \le \sum_{i\in I}\Phi(R_i) < 2n \enspace ,
  \]
  so $\sum_{i=1}^n r_i < 3n$.
  
  Finally, we observe that $Q_{i+1}$ contains at most one point each
  from $H^{\min}_i$, $H^{\max}_i$, $V^1_i$, and $V^2_i$, so
  \[
      \sum_{i=1}^{n}|Q_i| 
          \le |Q_1|+\sum_{i=1}^{n-1}(t_i+r_i+4) 
          \le 9 n \enspace . \qedhere
  \]
\end{proof}
...
appear in $H^{\max}_i$, $H^{\min}_i$ and $V^1_i$ and $V^2_i$.
...
%    \item $\ymax(S_{i+1}^*) \le \ymin(S_ii^*t)$;
%    \item $\ymax(S_{i+1}^*) \le \ymin(S_ii^*)$;
%  \end{enumerate}
%
% 
%  \[ \sum_{i=1}^{n}\left(|Q_i\cap V^i|+|Q_i\cap H^{\max}_i|+|Q_i\cap H^{\min}_i|\right) \in O(n) \enspace . \]
%\end{lem}
...
\begin{proof}
   ...
   Now, if $Q_{i+1}$ contains $t_i\ge 1$ points in $T_i$, then the fact
   that $Q_{i+1}$ is non-decreasing implies that $\ymin(S_{i+1})\le
   \ymin(S_i)-\lceil (t_i+1)/2 \rceil$.  This immediately implies that
   $\sum_{i=1}^{n-1} t_i \le 2n$.

   Similarly, if $Q_{i+1}$ contains $r_i\ge 1$ points in $R_i$, then
   the fact that $Q_{i+1}$ is non-decreasing means that $\Phi(R_{i+1})
   \le \Phi(R_i)-r_i+1$ or that $R_{i+1}\not\subseteq R_i$.  As in the
   previous proof, this implies that $\sum_{i=1}^n r_i \le 3n$.

   To account for points that are placed on horizontal lines
   $H^{\min}_1,\ldots,H^{\min}_{n-1}$, we use a similar, but simpler
   potential function argument.  Let $I=\{i\in\{1,\ldots,n-1\}:
   Q_{i+1}\cap T_i\neq\emptyset\}$ and observe that $H^{\min}_{i+1} =
   H^{\min}_i$ if and only if $i\not\in I$.

   It follows that, for distinct $i,j\in I$, $H^{\min}_i\cap
   \survivors(S_i)$ and $H^{\min}_j\cap\survivors(S_j)$ have no
   $x$-coordinates in common (see \figref{swords-nested-vi}).
   \begin{figure}
      \begin{center}
        \includegraphics{figs/swords-nested-hmin}
      \end{center}
      \caption{Counting points on $H^{\min}_1,\ldots,H^{\min}_{n-1}$.}
      \figlabel{swords-nested-vi}
   \end{figure}
   ...
   Counting the number of points in $V^1_1,\ldots,V^1_{n-1}$ is similar,
   except that we use the fact that, if $Q_{i+1}$ contains $v_i$ points
   in $V^1_i$, then all but one of these points kills an entire row of
   $R_i$.  This yields $\sum_{i=1}^{n-1}v_i \le 2n$.  Exactly the same
   argument shows that the number of points in $V^2_1,\ldots,V^2_{n-1}$
   is at most $2n$.

   Finally, it remains to count the contribution of points in
   $H^{\max}_1,\ldots,H^{\max}_n$.  The movement of the line
   $H^{\max}_i$ over time is a bit trickier.  If $i$ is a critical
   event, then $H^{\max}_i$ moves downwards, since $\ymax(B_{i+1}) <
   \xmin(S_{i+1}) \le \ymax(B_i)$.  Now note that, for distinct $i,j\in
   I$, $H^{\max}_i\cap \survivors(S_i)$ has no $x$-coordinates in common
   with $H^{\max}_j\cap \survivors(S_j)$.  Indeed, if $i< j$, then all
   $x$-coordinates in the former line are greater than $\ymin(S_i)$
   and all $x$-coordinates in the latter line are at most $\ymin(S_i)$.

   Now, if $Q_{i+1}$ contains $k_i$ points
   on $H^{\max}_i$, then each of these points kills the entire column
   one unit to its left. With the exception of the column killed by the
   leftmost point in $Q_{i+1}\cap H^{\max}_i$, this is the first time
   this column is killed by any point in any $Q_{j+1}\cap H^{\max}_j$
   for any $j\in\{1,\ldots,n-1\}$.  Therefore 
   \[
       \sum_{i=1}^{n-1}(|Q_{i+1}\cap H^{\max}_i|-1) \le n \enspace ,
   \]
   so the total contribution from points in $H^{\max}_1,\ldots,H^{\max}_{n-1}$
   is at most $2n$.

   Summing everything, we find that
   $\ex'(n,\{\swords,\nested\})\le\sum_{j=1}^n |Q_i|\le 13n$.
\end{proof}
...
\begin{proof}
  ...
  rest of the proof is similar to the proof of \thmref{nested-swords}.
\end{proof}
...
Our upper bound depends on a simple lemma about forbidden configurations
of points:
...
\begin{center}
   ...
   %\hspace{2cm}\includegraphics[height=3cm]{figs/lower-bounds-4}
\end{center}

\begin{proof}
   Let $S=\bigcup_{i=1}^n Q_i$.  We will show that, for each
   ...
   \]
   and let $p$ be the top rightmost point in $Q_i^*$, let $q$ be the
   highest point of $Q_i^*$ with $\x(q)=\xmin(Q_i^*)$, and let $r$ be the
   lowest point of $Q_i^*$ with $\x(q)=\xmin(Q_i^*)$.  Now, observe that
   $\killed(\{\nested,\ears,\david\}, \{p,q,r\}) \supseteq Q_i\setminus
   \{p,q\}$, so $p$ and $q$ are the only points of $Q_i$ that can appear again
   in $Q_{i+1},\ldots,Q_n$.
   \begin{figure}
     \begin{center}
       \includegraphics{figs/nested-ears-david}
     \end{center}
   ...
   that $\sum_{i=1}^n |Q_i| \le |S|+2n$.   All that remains now is to
   bound $|S|$.

   For each $i\in \{1,\ldots,n\}$, let $Q_i'$ be obtained from $Q_i$ by
   removing the leftmost point in each row.  Let $S'=\bigcup_{i=1}^n
   Q_i'$.  We claim that $S'$ satisfies the conditions of
   \lemref{forbidden}, so that $|S'|\le 2n$.  To see why this is so,
   suppose that $S'$ contains three points $a\in Q_i'$, $b\in Q_j'$,
   $c\in Q_k'$ satisfying the conditions of \lemref{forbidden}.
   Then the rules for $\nested$ and $\david$ imply that 
   \[
        i \le j \le k \le i
   \]
   so we must have $i=j=k$.  However, this contradicts the fact (due to
   $\nested$) that $Q_j$ is non-decreasing since the leftmost point of $Q_j$ 
   in the same row as $b$ is to the left of $b$.
   ...
   Thus, $|S|=|S'|+|S''| \le 4n$ and $\sum_{i=1}^{|Q_i|} \le 6n$.
\end{proof}
%\begin{proof}
%  Consider the set $Q_i$ played during some round $i$.  The rules
%  for $\nested$ imply that $Q_i$ is non-decreasing.  The rule for
%  $\bat$ implies that all but the top row of $Q_i$ is contained in
%  $R_i=\killed(\{\david\}, Q_i)$.   Indeed, all of $Q_i$ except its
%  leftmost column, bottom row, and (possibly) top row is in $R_i$
%
...
Bra\ss \cite{brass:turan}.  The following is essential ``proofs by figure'' in which a brief description of the solution is illustrated alongside the rules of each dot-puzzle.
...
we set $Q_i$ to be all the points of $Q'=\{n/2,\ldots,n\}^2$ on the line $\{(x,y):y=3n/2-x-i\}$.
\end{proof}
...
\begin{proof}
   We repeatedly take every second point on the diagonal $y=x-1$.
\end{proof}
