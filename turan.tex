...
\begin{proof}
  Let $x\in\{\taco,\bat,\nested,\crossing,\ears,\swords,\david\}$ be configuration not in $X$.  Label the vertices of our convex $n$-gon $1,\ldots,n$ in counterclockwise order.  We use the following constructions:
 \begin{enumerate}
...
\begin{obs}\obslabel{swords-region}
  ...
  \xmax(S),\, \ymax(S) \le y\le \xmin(S)\}$, a \emph{vertical line} $V$ passing
  through a point of $S$ with maximum $y$-coordinate,
  a \emph{horizontal line}
  $H^{\min}=\{(x,y)\in Q: y=\ymin(S)\}$ and a horizontal line $H^{\max}=\{(x,y)\in Q: y=\ymax(S)\}$.
\end{obs}
...
\begin{proof}
  The points defining $\ymax(S)$ and $\xmin(S)$ cover everything below
  $\ymax(S)$, except $H$, $V$, and $T$. The point defining $\ymax(S)$
  bounds $R$ from below, the point defining $\xmax(S)$ bounds $R$ from
  the left, and the point defining $\xmin(S)$ bounds $R$ from above.
\end{proof}
...
\begin{proof}
  ...
  \[
      \sum_{i=1}^{n}|Q_i| \le \sum_{i=1}^n(t_i+r_i+2) \le 8n \enspace . \qedhere
  \]
\end{proof}
...
are therefore asymptotically equivalent.  This comes from the
