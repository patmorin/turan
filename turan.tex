...
\author{Boris Aronov, Vida Dujmović, Ruy Fabila, Pat Morin, and Aurélien Ooms}
...
\begin{proof}
  ...
  \end{figure}
  Let $a\in S$ be a point with $\y(a)=\ymin(S)$.  Since $P$ is not in $H^{\min}$ and $p$ is not eliminated by $a$, we have 
  \begin{equation}
  ...
  Let $c\in S^*$ be the rightmost point in such that $y(c)=\ymax(S^*)$.
  Then, since $p$ is not in $V$ or $H^{\max}$,
  \[
      \y(p) > \ymax(S^*) \enspace .
  \]
  (\figref{survivors-proof}.c).
  Let $b\in S^*$ be a point with $\x(b)=\xmax(S^*)$.  Then
  \begin{equation}
      \text{$\x(p) \ge \xmax(S^*)$ or $\y(p) = \y(b)$} \enspace .
      \eqlabel{ac}
  \end{equation}
  (\figref{survivors-proof}.d).
  Taken together with the fact that $p\not\in R$, \eqref{aa}--\eqref{ac}
  imply that $\x(p)=\x(a)$ or that $\y(p) = \y(b)$. In the first case
  (\figref{survivors-proof}.e)
  $\x(p)=\x(a)$ and $\y(p) < \y(a)$ so $p$ would have to be in the
  region covered by $b$ and $V$.  In the second case, 
  (\figref{survivors-proof}.f)
  $\y(p) = \y(c)$
  and $\x(p) < x(c)$ so $p$ is contained in the region covered by $b$,
  $a$, $H^{\max}$, $H^{\min}$, and $V$.
\end{proof}
