...
In the following, for a point $q\in Q$, we use the notations
$\x(q)$ and $\y(q)$ to denote the $x$- and $y$-coordinates of $q$.
For $S\subseteq Q$, we use $\xmin(S)$ to denote the
minimum $x$-coordinate of any point in the point set $S$, i.e.,
$\xmin(S)=\min\{\x(q) : q\in S\}$.  
...
and any $S\subset Q$, we define $\killed(X,S)$ as the subset of $Q$ that can
no longer be played in the dot-puzzle game (for configurations in $X$)
if the points is $S$ have been played in previous rounds (these are the points
of $Q$ $\kappa$illed by $S$).  We also define
$\survivors(X,S)=Q\setminus\killed(X,S)$ to be the subset of points in $Q$
that can still be played in the dot-puzzle game if the points is $S$
have been played in previous rounds.



%We say that a set $S\subset Q$ is \emph{well-behaved} if $\ymax(S)
%\le \xmin(S)$.
%
%\begin{lem}\lemlabel{swords-region}
%  (Refer to \figref{survivors}.)  Let $S$ be any well-behaved subset
%  of $Q$.  Then $\survivors(\{\swords\},S)$ is contained in the union
%  of a \emph{triangular region} $T=\{(x,y)\in Q: x\le \ymin(S)+1\}$, a
%  \emph{rectangular region} $R=\{(x,y)\in Q: x\ge \xmax(S),\, \ymax(S)
%  \le y\le \xmin(S)\}$, a \emph{vertical line} $V$ passing through the
%  rightmost point of $S$ with maximum $y$-coordinate, a \emph{horizontal
%  line} $H^{\min}=\{(x,y)\in Q: y=\ymin(S)\}$ and a horizontal line
%  $H^{\max}=\{(x,y)\in Q: y=\ymax(S)\}$.
%\end{lem}
%
%\begin{proof}
%  The statement of the lemma is easily verified using the fact that,
%  because $S$ is well-behaved, $\ymin(S)\le \ymax(S)\le \xmin(S)\le
%  \xmax(s)$.
%\end{proof}
%
%\lemref{swords-region} gives us a form of \emph{potential function} that
...
%we define $S_i^*$, $T_i$, $R_i$, $V_i$, $H^{\max}_i$, and $H^{\min}_i$
%as in \lemref{swords-region}, but with respect to the set $S=S_i$.
%
%In general, the set $S_i$ will not be well-behaved and we need some way to deal with this.  Let $i$ the smallest index such that $S_i$ is not well-behaved so that $S_{i-1}$ is well-behaved, but $S_i$ is not.
%
...
\begin{proof}
  The proof, which follows, is a simple verification. Throughout, we use
  the fact that, by definition,
  \[
      \ymin(S) \le \ymax(S^*) \le \xmin(S) \le \xmax(S^*) \enspace .
  \]
  Suppose, for the sake of contradiction that there is some point
  \[
    p\in \survivors(\{\swords\},S) \cap \left(Q\setminus T\setminus R \setminus H^{\min} \setminus H^{\max}\setminus V\right) \enspace . 
  \]
  \begin{figure}
    \begin{center}
      \begin{tabular}{ccc}
         \includegraphics[width=.47\textwidth]{figs/survivors-proof-1} &
         \includegraphics[width=.47\textwidth]{figs/survivors-proof-2} \\
         (a) & (b) \\[1em]
         \includegraphics[width=.47\textwidth]{figs/survivors-proof-3} &
      ...
      \end{tabular}
    \end{center}
    \caption{The proof of \lemref{swords-region}.}
    \figlabel{survivors-proof}
  \end{figure}
  Throughout this proof, for a point $q\in Q$, we will use the shorthand
  $\killed(q)=\killed(\{\swords\},\{q\})$.
  Let $a\in S$ be a point with $\y(a)=\ymin(S)$.  Since $p\not\in T\cup H^{\min}\cup\killed(a)$, we have 
  \begin{equation}
      \text{($\y(p) > \ymin(S)$ and $\x(p)>\x(a)$)  or $\x(p) = \x(a)$} 
       \eqlabel{aa}
  \end{equation}
  (\figref{survivors-proof}.a).
  We also have
  \begin{equation}
      \y(p) \le \xmin(S) \eqlabel{ab}
  \end{equation}
  (\figref{survivors-proof}.b).
  Let $b\in S^*$ be the rightmost point in $S$ such that $y(b)=\ymax(S^*)$.
  Since $p\not\in V\cup H^{\max} \cup\killed(b)\cup\killed(a)$,
  \[
      \y(p) > \ymax(S^*) \enspace .
  \]
  (\figref{survivors-proof}.c).
  Let $c\in S^*$ be a point with $\x(c)=\xmax(S^*)$.  Then
  \begin{equation}
      \text{$\x(p) \ge \xmax(S^*)$ or $\y(p) = \y(c)$} \enspace .
      \eqlabel{ac}
  \end{equation}
  (\figref{survivors-proof}.d).
  Taken together with the fact that $p\not\in R$, \eqref{aa}--\eqref{ac}
  imply that $\x(p)=\x(a)$ or that $\y(p) = \y(c)$. In the first case
  (\figref{survivors-proof}.e)
  $\x(p)=\x(a)$ and $\y(p) < \y(a)$ so $p\in\killed(b)\cup V$, a contradiction.  In the second case, 
  (\figref{survivors-proof}.f)
  $\y(p) = \y(c)$
  and $\x(p) < x(c)$ so $p\in\killed(b)\cup\killed(a)\cup V\cup H^{\max}\cup H^{\min}$, also a contradiction.
\end{proof}

\lemref{swords-region} gives us a form of \emph{potential function}
that we can use.  We will apply it with $S=\bigcup_{j=1}^i Q_j$ so that
as rounds proceed, any time the value of $\xmax(S^*)$ or $\ymax(S^*)$
increases---or the value of $\xmin(S)$ decreases---the size of the
rectangle $R$ decreases. Any time the value of $\ymin(S)$ decreases,
the size of the triangle $T$ decreases.  So that we can refer to these
point sets as they change, we let $S_i=\bigcup_{j=1}^i Q_i$ and we
define $S_i^*$, $T_i$, $R_i$, $V_i$, $H^{\max}_i$, and $H^{\min}_i$
as in \lemref{swords-region}, but with respect to the set $S=S_i$.

The fact that $S^*$---which also determines two sides of $R$---is defined
in terms of $\xmin(S)$ is problematic. The following lemma helps us to
deal with that: \ldots

%
% The following lemma is not quite true
%\begin{lem}\lemlabel{survivors-normalize}
%  If $\xmin(Q_{i+1}) < \ymax(S_i)$, then $\survivors(X,S_{i+1})$
%  is contained in $T_{i+1}$ and a constant number of vertical and
%  horizontal lines.
%\end{lem}
%
%\begin{proof}
%  Refer to \figref{survivors-normalize}.
%  Let $a\in Q_{i+1}$ be any point with $\x(a)=\xmin(Q_{i+1})$.  Let
%  $b\in S_i$ be the leftmost point $S_i$ such that $\y(b)=\ymin(S_i)$.
%  Let $c\in S_i^*$ be any point with $y(c)=\ymax(S_i^*)$. Then it is
%  straightforward to verify that $\killed(X,\{a,b,c\})$ contains all
%  of $Q\setminus T_{i+1}$ except possibly the horizontal and vertical
%  lines through $a$, $b$, and $c$. (An important fact to remember during
%  this verification is that $\x(b) \ge \xmin(S_i) \ge \ymax(S_i^*)\ge \x(a)$.)
%  \begin{figure}
%    \begin{center}
%      \includegraphics{figs/survivors-normalize}
%    \end{center}
%    \caption{The proof of \lemref{survivors-normalize}.}
%    \figlabel{survivors-normalize}
%  \end{figure}
%\end{proof}
%
%Essentially, \lemref{survivors-normalize} tells us that $Q_{i+1}$ contains a point that removes any elements from $S^*$, then the rectangle $R$ stops being relevant. 
%
...
\begin{proof}
  ...
  $\ymin(S_{i+1}) \le \ymin(S_i)-t_i+2$.  This immediately implies that
  $\sum_{i=1}^n t_i \le 3n$.

  If there exists some $Q_i$ such that 
