...
The following lemma shows that we can, without losing much precision, study
...
\begin{proof}
  ...
  $a=(x_0,y_0)$, $b=(x_0,y_1)$, and $c=(x_1,y_1)$ with $y_0<y_1$ and
  $x_0<x_1$.  This implies that if we remove the rightmost point from
  every row of $S$, then at most one point remains in every column of $S$.
  It immediately follows that $|S|\le 2n$.
\end{proof}
...
\begin{thm}\thmlabel{nested-bat-david}
  $\ex'(n,\{\nested,\bat,\david\})\in O(n\log n)$.
\end{thm}

\begin{proof}
  First, imagine the game modified so that the $\david$-region extends
  upwards to infinity.  The rules for $\nested$ imply that $Q_i$ is
  non-decreasing.  The rule for $\bat$ implies that $Q_i$ is contained
  in the $\david$-region of the lowest-leftmost point in $Q_i$.

  Now, every point in $Q_i$ except the first and the last point in the
  bottom row completely kills its column and its row.  Since $Q_i$
  is non-decreasing, this means that the total number of row and
  columns killed is at least $|Q_i|-2$. It follows immediately that
  $\sum_{i=1}^{n}|Q_i| \le 4n$.

  Now, note that this upper bound depends only the size of $Q$.  The bound
  still holds even if the number of rounds were unlimited.  Partition the
  triangle $Q$ into squares in the obvious recursive way.  The preceding argument shows that 
\end{proof}
