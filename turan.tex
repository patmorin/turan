...
and we define $T_i$, $R_i$, $V_i$, $H^{\max}_i$, and $H^{\min}_i$ as in
\obsref{swords-region}, but with respect to the set $S=S_i$. So that $V_i$
is unambiguous, we define it as the vertical line passing through the
rightmost point in $S_i$ whose $y$-coordinate is equal to $\ymax(S_i)$.
